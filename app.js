...
app.get('/courseName', function (request, response) {  
    ...
    let sql = "SELECT id_course, name from course";
...
app.get('/finance', function(request, response) {
    let sql = "SELECT users.login, course.name, course.price FROM finance, users, course WHERE finance.id_user = users.id_user AND finance.id_course = course.id_course";
    db.all(sql, [], (err, rows) => {
        if (err) {
            throw err;
        }
...
app.post('/users', function(request, response) {
    let create_result = "INSERT INTO users (login, password) VALUES ('"+request.body.login+"','"+ request.body.password + "')"; 
    console.log(create_result);
    db.run(create_result, function(err) {
        if (err) {
          return console.error(err.message);
...
app.get('/result', function(request, response) {
    let sql = "SELECT users.login, result.result, course.name, result.code FROM result, users, course WHERE users.id_user = result.id_user AND course.id_course = result.id_course AND result.id_user = " + request.query.id_user;
    db.all(sql, [], (err, rows) => {
        if (err) {
            throw err;
        }
...
app.get('/resultCode', function(request, response) {
    let sql = "SELECT users.login, result.result, course.name, result.code FROM result, users, course WHERE users.id_user = result.id_user AND course.id_course = result.id_course AND result.code = '" + request.query.code +"'";
    db.all(sql, [], (err, rows) => {
        if (err) {
            throw err;
        }
